...
class Goods extends IController implements adminAuthorization
{
	...
	function goods_list()
	{
		...
		$goodsHandle->fields   = "distinct go.id,go.name,go.sell_price,go.market_price,go.store_nums,go.img,go.is_del,go.seller_id,go.is_share";
		...
		if(isset($_FILES['logo']['name']) && $_FILES['logo']['name']!='')
				{
					...
					$logo = $uploadObj->run();
					if(isset($logo['logo']['img']) && file_exists($logo['logo']['img']))
					{
						$_POST['logo'] = $logo['logo']['img'];
					}
				}
		$category_info = array(
			'name'      => $name,
